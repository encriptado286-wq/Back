...
class Registro {
  ...
  static async obtenerTodos() {
    ...
    return result.rows.map(
      (row) =>
        new Registro(row.id, row.alumno_id, row.fecha, row.pago)
    );
  }
...
router.get("/alumno/:id", async (req, res) => {
  try {
    ...
    console.error("❌ Error al obtener registros por alumno:", error);
    res.status(500).json({ error: "Error al obtener registros por alumno" });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    const actualizado = await Registro.actualizar(
      req.params.id,
      req.body
    );
